...
class Camera(object):
    ...
    def pixel2quant(self, pixel):
        ...
        d_all_x = np.sqrt(np.square(self.ru[0] - self.lu[0]) + np.square(self.ru[1] - self.lu[1])) + np.sqrt(np.square(self.rd[0] - self.ld[0]) + np.square(self.rd[1] - self.ld[1]))
        d_all_y = np.sqrt(np.square(self.lu[0] - self.ld[0]) + np.square(self.lu[1] - self.ld[1])) + np.sqrt(np.square(self.ru[0] - self.rd[0]) + np.square(self.ru[1] - self.rd[1]))
        d_all_x/=2.
        d_all_y/=2.
    ...
    def point_inside(self, p, p1, p2, p3, p4):
        ...
        area = self.triangle_area(p, p1, p2) + self.triangle_area(p, p2, p3) + self.triangle_area(p, p3, p4) + self.triangle_area(p, p4, p1)
    ...
    def optimize_center(self, img, x, y):
        ...
        for i in range(-search_width, search_width+1):
            for j in range(-search_width, search_width+1):
                mat = img[y+j-neighb_width: y+j+neighb_width+1, x+i-neighb_width: x+i+neighb_width+1]
