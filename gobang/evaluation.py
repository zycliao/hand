...
class evaluation(object):
    def __init__(self, SIZE):
        ...
        for i in range(self.SIZE):
            row = [(self.mid - max(abs(i - self.mid), abs(j - self.mid))) for j in range(self.SIZE)]
    ...
    def __analysis_horizon(self, board, i, j):
        ...
        for x in range(self.SIZE):
            line[x] = board[i][x]
        self.analysis_line(line, result, self.SIZE, j)
        for x in range(self.SIZE):
            if result[x] != TODO:
    ...
    def __analysis_vertical(self, board, i, j):
        ...
        for x in range(self.SIZE):
            line[x] = board[x][j]
        self.analysis_line(line, result, self.SIZE, i)
        for x in range(self.SIZE):
            if result[x] != TODO:
    ...
    def __analysis_left(self, board, i, j):
        ...
        k = 0
        while k < self.SIZE:
            if x + k > (self.SIZE-1) or y + k > (self.SIZE-1):
                break
    ...
    def __analysis_right(self, board, i, j):
        line, result, record = self.line, self.result, self.record
        TODO = self.TODO
        if (self.SIZE-1) - i < j: x, y, realnum = j - (self.SIZE-1) + i, (self.SIZE-1), (self.SIZE-1) - i
        else: x, y, realnum = 0, i + j, j
        k = 0
        while k < self.SIZE:
            if x + k > (self.SIZE-1) or y - k < 0:
                break
            line[k] = board[y - k][x + k]
            k += 1
        self.analysis_line(line, result, k, j - x)
