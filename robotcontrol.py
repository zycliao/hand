...
class Auboi5Robot:
    ...
    def set_relative_offset_on_base(self, relative):
        """
        * FUNCTION:    set_relative_offset_on_base
        * DESCRIPTION: 设置基于基座标系运动偏移量
        * INPUTS:      relative=(x, y, z) 相对位移，单位(m)
        * OUTPUTS:
        * RETURNS:     成功返回: RobotError.RobotError_SUCC
        *              失败返回: 其他
        * NOTES:
        ...
        self.check_event()
        if self.rshd >= 0 and self.connected:
            return libpyauboi5.set_relative_offset_on_base(self.rshd, relative)
        else:
            logger.warn("RSHD uninitialized or not login!!!")
            return RobotErrorType.RobotError_NotLogin

    def set_relative_offset_on_user(self, relative, user_coord):
        """
        ...
        * NOTES:
        ...
        self.check_event()
        if self.rshd >= 0 and self.connected:
            return libpyauboi5.set_relative_offset_on_user(self.rshd, relative, user_coord)
        else:
            logger.warn("RSHD uninitialized or not login!!!")
            return RobotErrorType.RobotError_NotLogin
...
def test(test_count):
    ...
    try:
        ...
        # 链接服务器
        port = 8899
        result = robot.connect(TEST_IP_ADD, port)
